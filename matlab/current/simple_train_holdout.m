...
function  [J, R, N, T, varData, chi2, pVars, val_chi2, val_pVars] = simple_train_holdout(data, inputs, max_epochs, val_epochs)
    % "NORMALIZE" DATA
    ...
    for epoch = 1:max_epochs
        ...
        for timepoint = 2:train_T-1 %time steps for each epoch. 
            ...
            H(:, timepoint+1) = JR;
            err = H(:, timepoint+1) - data(:, timepoint+1); %As in Andalman. Adata has entries every 0.5 s and JR at every 0.25 s.
        ...
        if val_counter > val_epochs
            fprintf('Validation loss stopped decreasing for 5 epochs in a row, so training was stopped.\n')
            break
        end
    end
